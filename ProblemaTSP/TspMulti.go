...
func crearMatrizAdyacencia(salida chan map[string]int) {
	...
	salida <- matrizAdyacencia
}

//Generamos las rutas para el multiarranque
func generarRutas(matrizAdyacencia map[string]int, salida chan [4][8]string) {
...
func costoTour(matrizAdyacencia map[string]int, rutas [4][8]string, salida chan [4]int) {
	var costos [4]int
	for i := 0; i < 4; i++ {
		var ruta []string
		for j := 0; j < 8; j++ {
...
//Generamos el costo de las rutas
func tourOptimizado(matrizAdyacencia map[string]int, rutas [4][8]string, salida chan [4][8]string) {
...
func main() {
	...
	canalMatrizA := make(chan map[string]int)
	canalRutas := make(chan [4][8]string)
	canalCosto := make(chan [4]int)
	canalRutasO := make(chan [4][8]string)
	canalCostoO := make(chan [4]int)

	go crearMatrizAdyacencia(canalMatrizA)
	matrizAdyacente := <-canalMatrizA

	go generarRutas(matrizAdyacente, canalRutas)
	rutas := <-canalRutas

	go costoTour(matrizAdyacente, rutas, canalCosto)
	costos := <-canalCosto
	...
	go tourOptimizado(matrizAdyacente, rutas, canalRutasO)
	rutasO := <-canalRutasO

	go costoTour(matrizAdyacente, rutasO, canalCostoO)
	costoO := <-canalCostoO
	...
	for i := 0; i < len(rutas); i++ {
		fmt.Print(rutasO[i])
		fmt.Println(" ", costoO[i])
	}

	numero := 0
	minimo := costoO[0]
	for i, valor := range costoO {
		if minimo > valor {
			numero = i
		}
	}
	fmt.Println("")
	fmt.Println("Ruta Óptima para recorrer el tour")
	fmt.Println("")
	fmt.Print(rutasO[numero])
	fmt.Println(" ", costoO[numero])
